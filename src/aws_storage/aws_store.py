...
from src.utils.my_utils import MODEL_S3_BUCKET_NAME
from src.exception.project_exception import MyException
from botocore.exceptions import ClientError
from pandas import DataFrame,read_csv
import pickle

Bucket = MODEL_S3_BUCKET_NAME
...
class SimpleStorageService:
    ...
    def s3_key_path_available(self, bucket_name, s3_key) -> bool:
        ...
        try:
            ...
            my_logger.error(f"Error checking S3 key path availability: {str(e)}")
    ...
    def get_bucket(self, bucket_name: str) :
        ...
        """
        my_logger.info("Entered the get_bucket method of SimpleStorageService class")
        try:
            bucket = self.s3_resource.Bucket(bucket_name)
            my_logger.info("Exited the get_bucket method of SimpleStorageService class")
            return bucket
        except Exception as e:
            raise MyException(e, sys) from e
    ...
    def get_file_object(self, filename: str, bucket_name: str) -> Union[List[object], object]:
        ...
        """
        my_logger.info("Entered the get_file_object method of SimpleStorageService class")
        try:
            bucket = self.get_bucket(bucket_name)
            ...
            my_logger.info("Exited the get_file_object method of SimpleStorageService class")
            return file_objs
        except Exception as e:
            raise MyException(e, sys) from e
    ...
    def load_model(self, model_name: str, bucket_name: str, model_dir: str = None) -> object:
        ...
        try:
            ...
            my_logger.info("Production model loaded from S3 bucket.")
            return model
        except Exception as e:
            raise MyException(e, sys) from e
    ...
    def create_folder(self, folder_name: str, bucket_name: str) -> None:
        ...
        Args:
            ...
            bucket_name (str): Name of the S3 bucket.
        """
        my_logger.info("Entered the create_folder method of SimpleStorageService class")
        try:
            ...
            my_logger.info("Exited the create_folder method of SimpleStorageService class")
    ...
    def upload_file(self, from_filename: str, to_filename: str, bucket_name: str, remove: bool = True):
        ...
        """
        my_logger.info("Entered the upload_file method of SimpleStorageService class")
        try:
            my_logger.info(f"Uploading {from_filename} to {to_filename} in {bucket_name}")
            self.s3_resource.meta.client.upload_file(from_filename, bucket_name, to_filename)
            my_logger.info(f"Uploaded {from_filename} to {to_filename} in {bucket_name}")

            # Delete the local file if remove is True
            if remove:
                os.remove(from_filename)
                my_logger.info(f"Removed local file {from_filename} after upload")
            my_logger.info("Exited the upload_file method of SimpleStorageService class")
        except Exception as e:
            raise MyException(e, sys) from e
    ...
    def upload_df_as_csv(self, data_frame: DataFrame, local_filename: str, bucket_filename: str, bucket_name: str) -> None:
        ...
        Args:
            ...
            bucket_name (str): Name of the S3 bucket.
        """
        my_logger.info("Entered the upload_df_as_csv method of SimpleStorageService class")
        try:
            # Save DataFrame to CSV locally and then upload it
            data_frame.to_csv(local_filename, index=None, header=True)
            self.upload_file(local_filename, bucket_filename, bucket_name)
            my_logger.info("Exited the upload_df_as_csv method of SimpleStorageService class")
        except Exception as e:
            raise MyException(e, sys) from e
    ...
    def get_df_from_object(self, object_: object) -> DataFrame:
        ...
        """
        my_logger.info("Entered the get_df_from_object method of SimpleStorageService class")
        try:
            content = self.read_object(object_, make_readable=True)
            df = read_csv(content, na_values="na")
            my_logger.info("Exited the get_df_from_object method of SimpleStorageService class")
            return df
        except Exception as e:
            raise MyException(e, sys) from e
    ...
    def read_csv(self, filename: str, bucket_name: str) -> DataFrame:
        ...
        """
        my_logger.info("Entered the read_csv method of SimpleStorageService class")
        try:
            csv_obj = self.get_file_object(filename, bucket_name)
            df = self.get_df_from_object(csv_obj)
            my_logger.info("Exited the read_csv method of SimpleStorageService class")
            return df
        except Exception as e:
            raise MyException(e, sys) from e